import { bot } from '../';
import { MessageEmbed } from '../lib';
import { IGuild } from '../models';

bot.on('guildCreate', async guild => {
    await bot.db_manager.models.Guild.create({ guildID: guild.id }).catch(err => console.error(err));
...
bot.on('guildDelete', async guild => {
    await bot.db_manager.deleteGuildData(guild.id);
