...
Queue.execute = async (bot, msg, args) => {
    ...
    if(server_queue && server_queue.player)
    {
        let queue_embed = new MessageEmbed(bot, msg.guild, false).setTitle('MUSIC QUEUE');

        server_queue.songs.forEach((song, index) => {
            let progress = (song == server_queue.playing.current) ? (`${bot.music_manager.secondsToDuration(Math.floor(1000 /* TODO: !!! */ / 1000))}/`) : ('');
        ...
        msg.channel.send({ embeds: [queue_embed] });
    }
    else
    {
        bot.deleteMsg(msg);
